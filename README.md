# Coastal Pure Water -- Client Site

**Client:** Brian Pretner
**Brand:** Coastal Pure Water
**Domain:** coastalwaterfl.com
**Market:** Palm Beach County, FL (municipal water only -- no well water pages)
**Draft URL:** https://draft--coastal-pure-water.netlify.app
**Built from:** `C:\Sites\_templates\water-filtration` (2026-07-28)

---

## IMPORTANT: Domain vs Brand Name Mismatch

The domain `coastalwaterfl.com` does NOT match the brand name "Coastal Pure Water." This is intentional per the client. Do not "fix" it.

---

## WARNING: A2P Opt-In Path -- UNDECIDED

No GHL form ID or chat widget ID has been provided. All pages carry placeholder comments:

- `<!-- GHL_FORM_ID: pending -->` -- water-assessment.html lead form
- `<!-- CHAT WIDGET: paste GHL chat widget snippet here. Widget ID: GHL_CHAT_WIDGET_ID pending -->` -- all other pages

**ZERO SMS consent language exists anywhere on the site.** Before going to production, choose one path:

- **CHAT WIDGET path:** GHL widget consent language is the opt-in. No SMS checkbox needed on any form.
- **WEB FORM path:** Add SMS consent checkbox with STOP/HELP language to water-assessment.html lead form only.

Do not go live until A2P path is decided and GHL IDs are wired in.

---

## Open Items for Steve

1. **GHL chat widget ID** -- paste into every page's `<!-- CHAT WIDGET -->` comment (excludes water-assessment.html, privacy-policy.html, terms-of-service.html)
2. **GHL form ID** -- wire into water-assessment.html embed
3. **Phone number** -- insert into every `<!-- PHONE SLOT: insert GHL tracking number here -->` comment (nav, footer, hero CTA)
4. **Email address** -- insert into every `<!-- EMAIL SLOT -->` comment
5. **Google Business Profile URL** -- insert into every `<!-- GBP_URL: pending -->` comment
6. **Owner headshot** -- drop a real photo of Brian into `images/` and wire into about.html at `<!-- HEADSHOT SLOT -->`. File must be named `coastal-pure-water-brian-pretner.webp`. Current layout is full-text (no photo displayed) until headshot is provided.
7. **A2P path decision** -- see WARNING above
8. **Real reviews** -- no testimonials or ratings on site until actual reviews exist. Trust band in place as interim.
9. **Credential/license line** -- currently "professional local installer." Confirm whether Florida license or certification applies; if so, update INSTALLER_CREDENTIAL_LINE in all pages.
...
| Token | Hex | Name |
|-------|-----|------|
| COLOR_PRIMARY | #0E7C8B | Deep Aqua |
| COLOR_ACCENT | #27A4B0 | Turquoise |
| COLOR_CTA | #F6C453 | Golden |
| COLOR_CTA_HOVER | #D1A747 | Golden dark |
| COLOR_LIGHT_FILL | #E5F4F6 | Light aqua tint |
| COLOR_BG | #F1F8F9 | Mist |
| COLOR_TEXT | #10303A | Deep Slate |
| COLOR_BORDER | #CFE5E8 | -- |
| COLOR_MUTED | #7B8D93 | -- |

Fonts: **Poppins** (headings), **Inter** (body) -- loaded from Google Fonts in each page `<head>`.

---

## Build Notes

- **Metro-only build.** No city landing pages. Service area page uses a metro-only callout. No `service-area/city-template.html` was deployed.
- **No well water pages.** `services/well-water-treatment.html` deleted. Well water footer link removed. `sitemap.xml` has no well water entry.
- **Favicon fill:** #0E7C8B (Deep Aqua). Generated via `node gen-favicons.mjs` using sharp + to-ico.
- **Logo:** converted from `Logo 8.png` (1774x887 RGB) to `images/logo.webp` and `images/logo-footer.webp` at quality 92.
- **Equipment spec terms:** all MWP brand names (Jacobi, ResinTech, CG10) and certification claims (NSF-certified, ceramic disc valve, 400-year, lifetime warranty) have been genericized. Run the equipment-spec grep before any production deploy to confirm zero residual hits.
- **No em dashes** anywhere in copy, comments, or commit messages. Use "and", "or", commas, or colons instead.
- **No credential claims.** Florida installer model not verified. "professional local installer" is the standing copy.
- **Flagship price:** $2,999 all-in installed.
- **Financing line:** "Flexible financing available, no credit check" (generic; no figure calculated).
...
`origin` must be `https://github.com/spotspid/coastal-pure-water-website.git` -- NOT the template repo.

---

## Files Removed vs Template

- `free-water-report.html` -- renamed to `water-assessment.html` (was already done in template; orphan file deleted)
- `services/well-water-treatment.html` -- municipal-only market, not needed
- `service-area/city-template.html` -- metro-only build, no city pages generated

---

*Template reference: `C:\Sites\_templates\water-filtration` (read-only; NEVER set as remote)*
